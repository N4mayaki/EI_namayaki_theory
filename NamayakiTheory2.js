...
var init = () => {
    ...
    // Lemma completion upgrade
    {
        let lemmaCost = new CustomCost((level) => {
            if (level == 0) return BigNumber.from(1e13);
    ...
    ///////////////////
    //// Story chapters    
    chapter1 = theory.createStoryChapter(0, "もう何も怖くない", 
        "初見クリアできなくて悔しいって？\n" +
        "大丈夫、失敗は既定路線だ\n" +
        "針の穴を通すような繊細な問題だ\n" +
        "果たしてたどり着けるだろうか？\n\n" +
...
var tick = (elapsedTime, multiplier) => {
    ...
    // 既に70に達しているかチェック
    if (time >= BigNumber.from(70)) {
        time = BigNumber.from(70); // 念のため70に固定
    ...
    if (nextTime > BigNumber.from(70)) {
        effectiveDt = BigNumber.from(70) - time; // 70までの残り時間
        time = BigNumber.from(70); // timeを70にクランプ
    } else {
        time = nextTime; // 70を超えないならそのまま更新
    }
    ...
    let numerator1 = (aVal * BigNumber.from(4) + bVal).pow(0.5);
    let numerator2 = (bVal + cVal * BigNumber.THREE).pow(0.5);
    let numerator3 = (aVal + cVal).pow(0.5);
    let denominator = BigNumber.from(Math.exp(0.061 * a.level + 0.016 * b.level + 0.028 * c.level));
    
    // A = (4a+b)^0.5 × (b+3c)^0.5 × (a+c)^0.5 / exp(0.061a + 0.016b + 0.028c)
    let A = numerator1 * numerator2 * numerator3 / denominator;
    
    // ρ̇ = 10^6 × t² × A × R(a,b,c)
    currency.value += effectiveDt * 1.02 * BigNumber.from(1e6) * time.pow(2) * A * resonance;
...
var getPrimaryEquation = () => {
    ...
    result += "\\\\ A = \\frac{(4a+b)^{0.5} (b+3c)^{0.5} (a+c)^{0.5}}{\\exp(0.061a + 0.016b + 0.028c)} \\\\";
...
var getSecondaryEquation = () => {
    return "\\text{Timelimit: }t=70,\\ \\ \\text{Goal: } \\rho \\geq 1e13";
}
